...
function begin_play() {
	...
	if (isEmpty(player_1.value) && isEmpty(player_2.value)) {
		alert("Two player game, both the field are mandatory");
		return 0;
	}
	player_1.value += "(X)";
	player_2.value += "(O)";
	...
	document.getElementById("turn_info").innerHTML = "Turns for: X";
	//document.getElementById("turn_start").innerHTML = "Turns for: ";
...
*/
function play() {
	var inputs = document.getElementById("move_text_id").value;
	var turn = document.getElementById("turn_info").innerHTML;
	
	
	
	if (!game_started()) {
		alert("Game is not Start");
		return;
	}
	
	if (inputs != "A1" && inputs != "A2" && inputs != "A3" && inputs != "B1" && inputs != "B2" && inputs != "B3" && inputs != "C1" && inputs != "C2" && inputs != "C3") {
		alert("Wrong Input!");
		return;
	}
	//var box = document.getElementById(inputs).innerHTML;
	var index_t = table_ids.indexOf(inputs);

	if (board_state[index_t] != '-1') {
		alert("This box has been taken!");
		return;
	}

	if (turn == "Turns for: X") {
		document.getElementById("turn_info").innerHTML = "Turns for: O";
		document.getElementById(inputs).innerHTML = "X";
		board_state[index_t] = 1;
	} else {
		document.getElementById("turn_info").innerHTML = "Turns for: X";
		document.getElementById(inputs).innerHTML = "O";
		board_state[index_t] = 2;
	}
	...
	if (board_state[0] == 2 && board_state[1] == 2 && board_state[2] == 2) {
		alert("winner is O");
		reset_play();
	}

	if (board_state[3] == 2 && board_state[4] == 2 && board_state[5] == 2) {
		alert("winner is O");
		reset_play();
	}

	if (board_state[6] == 2 && board_state[7] == 2 && board_state[8] == 2) {
		alert("winner is O");
		reset_play();
	}

	if (board_state[1] == 2 && board_state[4] == 2 && board_state[7] == 2) {
		alert("winner is O");
		reset_play();
	}

	if (board_state[2] == 2 && board_state[5] == 2 && board_state[8] == 2) {
		alert("winner is O");
		reset_play();
	}

	if (board_state[0] == 2 && board_state[4] == 2 && board_state[8] == 2) {
		alert("winner is O");
		reset_play();
	}
